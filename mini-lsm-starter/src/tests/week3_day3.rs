...
use crate::{
    ...
    tests::harness::{as_bytes, check_lsm_iter_result_by_key},
};
...
fn test_task2_lsm_iterator_mvcc() {
    ...
    let mut it = snapshot6.scan(Bound::Unbounded, Bound::Unbounded).unwrap();
    while it.is_valid() {
        println!("{:?}, {:?}", as_bytes(it.key()), as_bytes(it.value()));
        it.next().unwrap();
    }
    println!("----------------");
    check_lsm_iter_result_by_key(
